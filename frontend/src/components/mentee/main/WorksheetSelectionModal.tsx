...
export function WorksheetSelectionModal({
  ...
  // 칼럼이 하나만 있는 경우 자동으로 해당 칼럼 보기 모드로 전환 (파생값)
  const autoColumn = isOpen && items.length === 1 && items[0]?.type === 'COLUMN' ? items[0] : null;
  const effectiveViewingColumn = viewingColumn ?? autoColumn;
            ...
            {effectiveViewingColumn ? (
              ...
              <div className="flex flex-col h-full">
                <div className="p-6 border-b border-gray-100 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-lg font-bold text-slate-800 truncate">
                      {effectiveViewingColumn.columnTitle || '칼럼'}
                    </h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                  <HtmlContent
                    html={effectiveViewingColumn.columnContent || ''}
                ...
                <div className="p-6 pt-2 bg-gray-50/50">
                  <button
                    onClick={items.length > 1 ? () => setViewingColumn(null) : handleClose}
