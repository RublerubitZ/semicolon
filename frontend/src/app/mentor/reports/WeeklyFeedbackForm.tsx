...
export default function WeeklyFeedbackForm({ menteeId }: { menteeId: string }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 주간 총평 전체 + 특정 주차 데이터 동시 fetch
      const [feedbackRes, allFbRes, statsRes] = await Promise.all([
        fetch(
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          {/* 작성 현황 */}
          <div className="bg-white rounded-lg p-4 border">
            <p className="text-sm font-semibold text-gray-700 mb-2">주간 총평 작성 현황</p>
              ...
              {Array.from({ length: totalWeeks }, (_, i) => i + 1).map((week) => {
                ...
                return (
                  <div key={week} className="flex items-center justify-between text-sm">
                    ...
                    <span className={hasData ? 'text-green-600 font-medium' : 'text-gray-400'}>
                      {hasData ? '작성 완료' : '미작성'}
                    </span>
                  </div>
                );
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-700">
              {selectedWeek}주차 — {feedback ? '총평 수정' : '총평 작성'}
            </h3>
            {feedback && (
